            ...
            (items as TeachingEngagement[]).map((item, index) => (
              <div key={index} className="border-b border-gray-300 pb-4 last:border-b-0">
                <div className="flex justify-between items-start gap-4">
                  ...
                  <span className="text-sm text-gray-600 whitespace-nowrap">{item.season}</span>
                </div>
              </div>
            ))}
              ...
              <div key={index} className="border-b border-gray-300 pb-4 last:border-b-0">
                ...
                <div className="space-y-1 text-sm">
                  <p className="text-gray-700">
                    <span className="font-semibold">Period:</span> {item.dateRange}
                  </p>
                  <p className="text-gray-700">
                    <span className="font-semibold">Scholar:</span> {item.scholars}
                  </p>
                  {item.otherSupervisors && (
                    <p className="text-gray-700">
                      <span className="font-semibold">Other Supervisors:</span> {item.otherSupervisors}
                    </p>
                  )}
                </div>
              </div>
